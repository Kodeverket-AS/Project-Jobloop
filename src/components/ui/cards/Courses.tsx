import { LesMerButton } from '@/components/buttons';
import { useTranslations } from 'next-intl';
import Image, { type StaticImageData } from 'next/image';

interface CoursesProps {
  image: StaticImageData;
  title: string;
  text: string;
  path: string;
  alt: string;
}

// TODO: Deal with double link issue.
// TODO: Remove divs where possible.
// TODO: Translate aria-labels.
export function CoursesCard({ image, title, text, path, alt }: CoursesProps) {
  const t = useTranslations("dictionary")
  return (
    <section
      aria-labelledby={`course-${title.replace(/\s+/g, '-').toLowerCase()}`}
      className={`
        group flex flex-col w-full gap-4 lg:gap-6 xl:flex-row-reverse bg-white
        rounded-xl shadow-xs hover:shadow-md border border-gray-50 p-6
        transition-all duration-200 justify-between
      `}
    >
      <div className='flex flex-col w-full gap-4 xl:w-1/2 justify-center order-last'>
        <h3
          id={`course-${title.replace(/\s+/g, '-').toLowerCase()}`}
          className='text-xl text-kv-black md:text-2xl font-semibold leading-tight'
        >
          {title}
        </h3>
        <p className='text-base text-gray-600 md:text-lg leading-relaxed'>
          {text}
        </p>
        <div className='pt-2'>
          <LesMerButton
            Path={path}
            Text={t("readMore")}
            Aria={`Les mer om ${title}`}
          />
        </div>
      </div>
      <a href={path} className='w-full h-80 xl:w-1/2 relative overflow-hidden rounded-xl order-first'>
        <Image
          alt={alt}
          src={image}
          width={600}
          height={600}
          className='object-cover h-full max-w-full'
        />
      </a>
    </section>
  );
}
